...
echo $key | sudo -S sed -i 's/GRUB TIMEOUT="[^"]*"/GRUB TIMEOUT="10"/' /etc/default/grub
...
if ! echo $key | sudo -S grep -q "GRUB_GFXMODE=1024x768x16" /etc/default/grub; then		
  ...
  echo "Grub_GFXMODE is already configured for CSI Theme" | tee -a "$output_file"
fi
...
if ! echo $key | sudo -S grep -q "GRUB_GFXPAYLOAD_LINUX=keep" /etc/default/grub; then		
  ...
  echo "Grub_GFXPAYLOAD is already configured for CSI Theme" | tee -a "$output_file"
fi
